...
**nuthatch**(䴓)是一种把种子塞进树皮缝里囤着、还记得藏在哪、头朝下爬树的小鸟。代号取自它的两个本事——**囤食 + 记住位置**——正是这个项目的护城河:**用「使用历史」学出来的逐专家流式缓存**。
...
- **C++ 而非 Go**:ggml 是 C/C++,MoE 前向是紧密的张量算子调用,跨 FFI 得不偿失。Go 适合写辅助工具,主体留 C++。
- **ggml 走 vcpkg,不自己写内核**:护城河在**算法**(缓存策略),不在**引擎**(矩阵乘法 ggml 已经做到极致)。复用成熟内核,把精力花在差异化上。
- **RE2 → PCRE2**:分词器预分词要跑 OLMo 的正则(带 `\p{L}` Unicode 属性 + 前瞻 `(?!\S)`)。RE2 新版会拖进 abseil,在 macOS 上撞 `CoreFoundation` 链接坑;PCRE2 **自包含、且支持前瞻**,能一次跑完整正则。**"引入依赖前先讨论"这条规矩,这次省了大麻烦。**
