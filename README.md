...
**且非 OLMoE 专属**:换到 IBM **granite3-moe**(完全不同的架构,24 层 / 32 专家,经 llama.cpp 导出路由)同样成立——learned 赢 LRU/OS **+8.8 ~ +17.7 pp**,同样"预算越紧越赢"。见 [`docs/cross-arch-trace.md`](docs/cross-arch-trace.md)。
...
想系统地读懂它,见 **[`docs/analysis/`](docs/analysis/index.md)** —— 11 章按建造顺序拆解:架构哲学 → 构建/CI → 读 GGUF 与 pread → 研究先行 → OLMoE 前向 → 对拍与 norm_topk → 分词器 → KV cache → **护城河** → 物理流式 → 工程判断与边界,每个论断链到源码。(另有单篇 [`docs/blog-draft.md`](docs/blog-draft.md)。)
